  ...
  if (orders[index].market === tradingPair) {
    ...
    if (orders[index].side === 'bid' && orders[index].orderType === 'limit') {
      if (orders[index].volume - orders[index].remainVol === 0 && orders[index].state === 'wait') {
        // first record
        Account.makerBuyBalance = new Decimal(Account.makerBuyBalance).minus(orders[index].volume)
        console.log(`updated minus makerBuyBalance : ${orders[index].volume}`)
      } else if (orders[index].remainVol === 0 && orders[index].state === 'done') {
        Account.makerSellBalance = new Decimal(Account.makerSellBalance).plus(orders[index].volume)
        Account.makerVol = new Decimal(Account.makerVol).plus(orders[index].volume)
        console.log(`updated plus makerSellBalance and volume : ${orders[index].volume}, ${orders[index].volume}`)
      }
      // let currentBid = new Decimal(orders[index].price.toString())
      // if (currentBid.minus(BestBid) <= -0.001) {
      //   let param = {
      //     id: orders[index].id // number
      //   }
      //   const cancel = new RestV2()

      //   cancel.cancelOrder(param)
      //     .then((response) => {
      //       console.log(`cancel order by ws : ${response.id}`)
      //       Account.makerBuyBalance = new Decimal(Account.makerBuyBalance.toString()).plus(response.volume)
      //     })
      //     .catch((error) => {
      ...
      if (orders[index].volume - (orders[index].remainVol) === 0 && orders[index].state === 'wait') {
        // console.log('2')
        Account.makerSellBalance = new Decimal(Account.makerSellBalance).minus(orders[index].volume)
        console.log(`updated minus makersellBalance : ${orders[index].volume}`)
      } else if (orders[index].remainVol === 0 && orders[index].state === 'done') {
        Account.makerBuyBalance = new Decimal(Account.makerBuyBalance).plus(orders[index].volume)
        Account.makerVol = new Decimal(Account.makerVol).plus(orders[index].volume)
        console.log(`updated plus makerBuyBalance and Volume : ${orders[index].volume}, ${orders[index].volume}`)
      }
      // let currentAsk = new Decimal(orders[index].price.toString())
      ...
      //     .then((response) => {
      //       console.log(`cancel order by ws: ${response.id}`)
      //       Account.makerSellBalance = new Decimal(Account.makerSellBalance.toString()).plus(response.volume)
      //     })
      //     .catch((error) => {
      //       console.error(error.message)
      //     })
